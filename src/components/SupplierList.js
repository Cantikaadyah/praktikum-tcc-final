...
const SupplierList = () => {
  ...
  const getSuppliers = async () => {
    const response = await axios.get("https://backend1test1-gxdo4hmr6q-et.a.run.app/suppliers");
    setSupplier(response.data);
  };

  const deleteSupplier = async (id) => {
    try {
      await axios.delete(`https://backend1test1-gxdo4hmr6q-et.a.run.app/suppliers/${id}`);
      getSuppliers();
    } catch (error) {
      console.log(error);
    }
